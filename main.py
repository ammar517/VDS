    ...
    with get_cursor(commit=True) as cur:
        cur.execute(
            """INSERT INTO projects
               (id, name, project_type, main_file, source_path, venv_path,
                status, pid, auto_start, created_at, updated_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            (
                pid_row["id"], pid_row["name"], pid_row["project_type"],
...
# ════════════════════════════════════════════════════════════════════════════
# 4) YETKI KONTROLU -- sadece OWNER_ID
# ════════════════════════════════════════════════════════════════════════════
...
def owner_only(handler):
    ...
    def wrapper(update, *args, **kwargs):
        ...
        if user_id != OWNER_ID:
            kex_logger.warning("Yetkisiz erisim denemesi -> user_id=%s zaman=%s", user_id, now_iso())
            try:
                chat_id = update.chat.id if hasattr(update, "chat") else update.message.chat.id
                bot.send_message(chat_id, "⛔ Bu bot size özel değildir.")
            except Exception:
                pass
            return None
...
def build_projects_keyboard(page: int = 0) -> types.InlineKeyboardMarkup:
    projects = list_projects()
...
def get_welcome_text() -> str:
    return (
        ...
        f"{BRAND_NAME} — sunucunu tek panelden yönetmen için burada. "
        f"Aşağıda sunucunun anlık durumunu görüyorsun; menüden proje "
        f"yükleyebilir, çalıştırabilir, loglarını izleyebilir ve yedek "
        f"alabilirsin.\n"
    )
...
def cmd_start(message):
    ...
    log_event("bot_menu", "Owner ana menuyu acti")
...
def cb_refresh(call):
    try:
        bot.edit_message_text(get_system_status_text(), call.message.chat.id, call.message.message_id,
                               reply_markup=get_main_menu_keyboard())
    except Exception:
        pass
...
def cb_upload_info(call):
    bot.answer_callback_query(call.id)
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🤖 Projelerim", callback_data="projects_page:0"))
    kb.add(types.InlineKeyboardButton("🏠 Ana Menü", callback_data="menu_refresh"))
    bot.send_message(
        call.message.chat.id,
        "📥 <b>YENİ PROJE YÜKLEME</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━\n"
...
def cb_help(call):
    ...
    bot.send_message(
        ...
        "🤖 <b>PROJELERİM</b> — yüklediğin tüm projeleri listeler, tıkla → yönet\n"
        "📥 <b>YÜKLE</b> — dosya gönderip yeni proje ekleme rehberi\n"
        "💾 <b>YEDEKLER</b> — proje bazlı zip yedek alma / indirme / silme\n"
...
def cb_backups_menu(call):
    ...
    kb.add(types.InlineKeyboardButton("🤖 Projeden Yedek Al", callback_data="projects_page:0"))
    kb.add(types.InlineKeyboardButton("🏠 Ana Menü", callback_data="menu_refresh"))
    text = (
        "💾 <b>YEDEKLER</b>\n━━━━━━━━━━━━━━━━━━━━━\n"
        "Yeni yedek almak için bir projeye gir → 💾 YEDEK AL.\n\n"
...
def _resolve_backup_by_index(idx_str: str):
    try:
        idx = int(idx_str)
    except ValueError:
        return None
    backups = list_backup_files()[:10]
...
def cb_backup_view(call):
    ...
    backup_path = _resolve_backup_by_index(idx_str)
    bot.answer_callback_query(call.id)
...
def cb_projects_page(call):
    ...
    bot.answer_callback_query(call.id)
    text = "🤖 <b>PROJELERİM</b>\n━━━━━━━━━━━━━━━━━━\nBir projeye tıkla veya .py / .zip / .js dosyası gönderip yeni proje yükle."
    try:
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id,
                               reply_markup=build_projects_keyboard(page))
    except Exception:
        bot.send_message(call.message.chat.id, text, reply_markup=build_projects_keyboard(page))
...
def cb_project_delete_confirm(call):
    ...
    try:
        bot.edit_message_text("✅ Proje silindi.", call.message.chat.id, call.message.message_id,
                               reply_markup=build_projects_keyboard(0))
    except Exception:
        bot.send_message(call.message.chat.id, "✅ Proje silindi.", reply_markup=build_projects_keyboard(0))
...
def handle_document(message):
    if OWNER_ID in _upload_locks and _upload_locks[OWNER_ID]:
    ...
    _upload_locks[OWNER_ID] = True
    threading.Thread(target=_process_upload, args=(message, doc, file_name, ext, status_msg), daemon=True).start()
...
def _process_upload(message, doc, file_name, ext, status_msg):
